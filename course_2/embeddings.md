...
## Token embeddings
**Token embedding**: a list of floating-point numbers that represents a token. 
Each token gets its own unique coordinate in a high-dimensional space.

An example lookup table might include:
...
### Learning embeddings
- The embedding table starts as a matrix of random numbers. 
- The model learns that to make better predictions during training and nudges the vectors around.
...
while preserving the original neighborhood structures. 
There is some distortion, but it gives you a very good idea of which continents are neighbors. 
